...
<template>
  <div class="wrap">
    ...
    <div class="max-w-4xl mx-auto mt-8 mb-12 sm:mt-10">
      <div class="text-lg font-bold sm:text-sm text-center">這次帶來的工作坊從10本繪本發想，分成分程是「繪本與人文」、「科學與創造」兩個主軸。繪本與人文由流寓工作室與果實玩樂生活實驗室兩個教育推廣團隊， 以「自然接觸」、「童趣手作」為課程精神，帶領以自然界的材料或觀察，帶領孩子與大人們進行手作課程。「科學與創造」則由光試所與Pixelight以「鏡面反射」 、 「電路電學」、「虛擬實境」為設計精神，帶領孩子與大人們進行科學與AR、VR的奇妙視覺體驗。</div>
    </div>
    <div class="flex flex-wrap justify-between xl:justify-around max-w-5xl mx-auto">
      <ActivityItem v-for="(i, index) in meetingList" :href="'/activities/' + $route.params.activityID + '/' + index" :img="'Data/Activities/' + $route.params.activityID + '/' + index + '/cover.jpg'" :typeName="i.typeName" :class="{ invisible: i.title === 'none' }">
