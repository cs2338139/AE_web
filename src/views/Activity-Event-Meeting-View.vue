  ...
  methods: {
    LoadJson() {
      ...
      if (activityID && eventID) {
        axios
          .get("Data/" + activityID + "/" + eventID + "/MeetingContent.json")
          .then((response) => {
    ...
    <div class="wrap">
      ...
      <div class="grid grid-cols-5 gap-x-16 gap-y-10 h-960px">
        ...
        <div class="col-start-1 col-end-4 row-start-1 row-end-4">
          <ImageBox :path="this.$route.params.activityID + '/' + this.$route.params.eventID" :img="meetingData.imgs" :time="3000" :auto="false" />
        </div>

        <div class="col-start-4 col-end-6 row-start-1 row-end-4 col-start">
          <div class="w-4/5 h-full">
            <div class="mt-3 mb-16">
              <div class="mb-3 text-4xl font-bold">{{ meetingData.date }}</div>
              <div class="text-3xl font-normal">{{ meetingData.time }}</div>
            </div>

            <ul class="text-lg list-disc list-outside">
              <li>參加人數：{{ meetingData.people }}</li>
              <li>活動地點：{{ meetingData.place }}</li>
            </ul>
            ...
            <div class="mb-16">
              ...
              <a :href="meetingData.link" target="_blank" class="my-2 text-xl font-bold text-zinc-500"><div>報名表單｜點我報名</div></a>
